---
title: "All species simulation analysis"
subtitle: "Before ODD updated code"
...
Set input and output folders, specific id and get file
```{r}
indir <- file.path("/home/ludmilla/Documents/uni_wuerzburg/phd_project/thesis/model/inputs") #CCTBserver: /home/luf74xx/Dokumente/model/EDoutputs
inid <- file.path("spinput.csv") # check in simID file in output
...
outid <- file.path("annualfullsp") #simID
...
3. Plot individual biomass, weekly
...
weeksize <- outdata %>%
  group_by(week, sp) %>%
  summarise(rad.mean = 3*mean(radius), rad.sd = 3*sd(radius)) #each cell is 3x3cm²
...
4. Biomass density over time
 Re-scale density cover abundance to check results 