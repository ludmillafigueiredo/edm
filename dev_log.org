...
- [ ] Check orgstable format: Julia DB?
  - [ ] Make sure it is ready for R analysis
...
*newOrg()* will have multiple methods: One for initialization, where individuals always establish and another where it depends on the individuals fitness in the grid and establishment!() is called.  
 ...
 - [ ] Simulate single functional group
   ...
   - [ ] wind and ant pollination (see *reproduction()*): Bullock et al. 2017: 
...
- [ ] Wind and ant dispersal kernels: ExP and Log-Sech distributions not in Julia. Having the pdf, it is necessary to find the inverse of it. For some pdfs, it might not exist (if they are bijective funcitons).
Possibilities:
